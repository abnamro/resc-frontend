<template>
  <div class="flex text-start">
    <!-- Finding info -->
    <div class="w-1/3 px-2 py-4 flex flex-col gap-1">
      <div>
        <span class="font-bold">Commit ID: </span>
        <a class="cursor-pointer text-primary-emphasis underline hover:text-primary-emphasis-alt"
          :href="props.finding.commit_url ?? ''" target="_blank">
      ...
      <div>
        <span class="font-bold text-ellipsis">Commit Message: </span>{{ props.finding.commit_message }}
      </div>
    ...
    <Tabs value="0" class="w-2/3">
      ...
      <TabPanels>
        ...
        <TabPanel value="2">
          <RuleTab :ruleName="props.finding.rule_name" :rulePack="props.finding.rule_pack"></RuleTab>
        </TabPanel>
      </TabPanels>
    </Tabs>
  </div>
